...
export class ListsController {
  ...
  // 2. 리스트 작성
  @Post('/board/:bid/list')
  async createList(@Param('bid') bid: number, @Body() createListsDto: CreateListsDto) {
  ...
  async updateList(@Param('bid') bid: number, @Param('lid') lid: number, @Body() updateListsDto: UpdateListsDto) {
  ...
  async deleteList(@Param('bid') bid: number, @Param('lid') lid: number) {
  ...
  @Patch('/board/:bid/order')
  async changeLists(@Param('bid') bid: number, @Body() listsDto: ListsDto) {
    const change = await this.listsService.changeLists(bid, listsDto.lid, listsDto.newPosition);
    return { message: '리스트 순서가 성공적으로 변경되었습니다.', data: { change } };
  }
}
