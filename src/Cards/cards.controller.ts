...
export class CardsController {
  ...
  // 7. 카드 순서변경 : Patch : localhost:3000/api/list/:lid/order
  // 리스트아이디(lid)는 params에 받고, body에 변경할 카드아이디(cid), 현재순서(position), 바꾸고싶은순서(newPosition)를 넣는다.
  @Patch('/list/:lid/order')
  async changeCards(@Param('lid') lid: number, @Body() orderDto: OrderDto) {
    const change = await this.cardService.changeCards(lid, orderDto.lid, orderDto.cid, orderDto.newPosition);
    return { message: '카드 순서가 성공적으로 변경되었습니다.', data: { change } };
  }
}
